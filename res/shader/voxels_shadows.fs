...
float lookup( vec2 offSet)
	{
		...
		return shadow2DProj(ShadowMap, ShadowCoord +
		                               vec4(offSet.x * 1 * ShadowCoord.w,
		                                    offSet.y * 1 * ShadowCoord.w,
		                                    0.05,
		                                    0.0) ).w;
	}
...
const vec4 fogcolor = vec4(0.0, 0.15, 0.3, 1.0);
...
void main(void) {
    ...
    if(blockdata.x > 0) {
        ...
        Texture.y = texcoord.z;
    } else {
        Texture.x = fract( texcoord.x + texcoord.z );
        Texture.y = texcoord.y;
    }
    Texture.x = (Texture.x + texcoord.w)/size.x;
    vec3 pos = blockdata.yzw;
    vec3 fragment = sh_light( pos, beach);
...
float shadow;
    ...
    gl_FragColor = (shadow+0.2) * mix( fogcolor, color * vec4( fragment, 1), fog);
